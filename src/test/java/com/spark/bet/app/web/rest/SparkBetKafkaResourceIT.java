...
class SparkBetKafkaResourceIT {
    ...
    void setup() {
        ...
        SparkBetKafkaResource kafkaResource = new SparkBetKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/spark-bet-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/spark-bet-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
